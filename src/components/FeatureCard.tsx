...
export const Card = ({ icon, title, description, delay }:{title:any,description:any,icon:any,delay:number}) => {
  return (
    <motion.div
  ...
  >
    <div className="flex flex-col items-center text-center">
      ...
      <h3 className="text-xl md:text-2xl font-bold text-purple-800 mb-4">{title}</h3>
    ...
    <div className="py-16 px-6 bg-gradient-to-b from-purple-50 to-white">
      <div className="container mx-auto grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {cards.map((card, index) => (
          <Card key={index} icon={card.icon} title={card.title} description={card.description} delay={index * 0.2} />
        ))}
      </div>
    </div>
  );
