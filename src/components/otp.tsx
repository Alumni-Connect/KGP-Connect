...
import React, { useState, useEffect, useRef, ChangeEvent, KeyboardEvent, ClipboardEvent } from 'react';
import { useRouter } from 'next/navigation';

const OTPInputWithTimer = ({changeForm,email,password}:{changeForm:() => void,email:any,password:any}) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 max-w-md mx-auto bg-white rounded-xl shadow-md">
      <div className="w-full text-center mb-6">
        <button className='flex items-center justify-center' onClick={changeForm}><Undo2></Undo2></button>
      ...
      <div className="flex flex-col items-center w-full mb-4">
        <div className={`text-xl font-medium ${timer <= 10 ? 'text-red-500' : 'text-gray-700'}`}>
          {formatTime(timer)}
        </div>
