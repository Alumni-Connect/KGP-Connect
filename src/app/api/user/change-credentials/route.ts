...
export async function POST(req:Request){
    ...
    try{
       ...
       return NextResponse.json({msg:"sorry! no otp is generated at this moment"},{status:400})
        }
...
export async function PUT(req:Request){
    ...
    if(!verifyOtp){
        return NextResponse.json({msg:"sorry! no otp is found with this email"},{status:400})
    }

    if( verifyOtp.expires< new Date()){
        console.log(new Date(Date.now()+60*60),new Date(),verifyOtp.expires)
        return NextResponse.json({msg:"otp is being expired"},{status:400})
    }
