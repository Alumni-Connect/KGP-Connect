...
export default function JobTable({ data }: { data: any }) {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await fetch(`/api/jobboard-admin?id=${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item.status === "open" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                    >
                      {item.status}
                    </span>
                  </td>
