...
//criterion
  const addCriterion = () => {
    setFormData((prev)=>{ return {...prev,criteria:[...prev.criteria, '']}});
  };

  const removeCriterion = (indexToRemove:number) => {
    setFormData({...formData,criteria: formData.criteria.filter((_, index) => index !== indexToRemove)});
  };
  ...
  useEffect(()=>{
    if(params.update!==undefined && params.update[1]!==undefined){
       fetch(`http://localhost:3000/api/scholarships/adminsOperation?scholarshipId=${params.update[1]}`)
      .then(async(res)=>{
        if(res.status===200){
          ...
          const scholarship=response.scholarship
           setFormData(()=>{ return {
            ...
            const formQuestion=scholarship.formQuestions.map((formQuestion:any)=> {return { 
              type : formQuestion.type,
              question : formQuestion.description,
              required: formQuestion.required,
              options: formQuestion.options.map((option:string,index:number)=> { return {id:index,text:option}}),
              isDropDown : false
            }})
                ...
                try {
                  ...
                  const queryFormQuestion=question.map((question)=> { 
                    return {
                      ...
                      options: question.options.map(option=> {console.log(option); return option.text.includes("Option")? '': option.text}) 
                    }})
                  const query={
                  ...
                  let response
                  if(params.update!==undefined && params.update[1]!==undefined){
                   response = await fetch('/api/scholarships/adminsOperation', {
                    method: 'PUT',
                    headers: {
